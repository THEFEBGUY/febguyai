...
def message_to_payload(message: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["role"] = payload.get("role") or "ai"
...
def row_to_code_chat(
    ...
    chat_item["projectFiles"] = load_code_project_files(row["profile_id"], row["id"], conn, scope)
    return chat_item
        ...
        for token, session in sessions.items():
            ...
            conn.execute(
                ...
                (
                    ...
                    session.get("last_seen_at") or now_iso(),
                ),
            )
        ...
        with db_connect() as conn:
            ...
            for token, session in sessions.items():
                ...
                conn.execute(
                    """
                    INSERT INTO sessions
                        (token, profile_id, mode, guest_id, device_id, created_at, last_seen_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(token) DO UPDATE SET
                        profile_id = excluded.profile_id,
                        mode = excluded.mode,
                    ...
                    (
                        token,
                        session.get("profile_id"),
                        session.get("mode") if session.get("mode") in {"guest", "profile"} else "profile",
    ...
    with DATA_LOCK:
        with db_connect() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (token, profile_id, mode, guest_id, device_id, now_iso(), now_iso()),
            )
    return token
        ...
        with db_connect() as conn:
            ...
            if profile is None:
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, NULL, ?, ?)
                    """,
                    (guest_id, normalized_device_id, profile_id, timestamp, timestamp),
                )
            ...
            conn.execute(
                ...
                DELETE FROM sessions
                WHERE token = (
                    SELECT session_token FROM guest_sessions WHERE guest_id = ?
                )
                """,
                (guest_id,),
            )
            ...
            conn.execute(
                ...
                SET session_token = ?, last_seen_at = ?
                WHERE guest_id = ?
                """,
                (session_token, timestamp, guest_id),
            )

    ensure_profile_files(profile_id)
    refreshed_profile = find_profile(profile_id) or profile
    refreshed_profile["is_guest"] = True
    refreshed_profile["guest_id"] = guest_id
    log_activity(profile_id, "guest_session_started", {"mode": "guest"})
    return {
        "profile": public_profile(refreshed_profile),
        "token": session_token,
        "session_mode": "guest",
        "guest": {"id": guest_id},
    }
        ...
        with db_connect() as conn:
            ...
            for limit_key in requested_keys:
                ...
                usage_row = conn.execute(
                    ...
                    """,
                    (guest_row["guest_id"], normalized_device_id, limit_key),
                ).fetchone()
            ...
            for limit_key in requested_keys:
                ...
                conn.execute(
                    ...
                    INSERT INTO usage_limits
                        (id, guest_id, device_id, limit_key, used_count, max_count, created_at, updated_at)
                    VALUES (?, ?, ?, ?, 1, ?, ?, ?)
...
def log_activity(
    ...
    try:
        ...
        with DATA_LOCK:
            with db_connect() as conn:
                scope = ownership_scope_for_profile(profile_id, conn) if profile_id else None
                conn.execute(
                    """
...
def enforce_device_bound_session_access(
    ...
    with db_connect() as conn:
        row = conn.execute(
            """
            SELECT profiles.device_id
            FROM sessions
            JOIN profiles ON profiles.id = sessions.profile_id
            WHERE sessions.token = ?
                AND sessions.mode = 'profile'
                AND profiles.profile_kind = 'account'
            """,
            (token,),
        ).fetchone()

    if row and (not device_id or row["device_id"] != device_id):
...
def get_session_context(authorization: str | None) -> dict[str, Any]:
    ...
    with db_connect() as conn:
        session = conn.execute(
            """
            SELECT token, profile_id, mode, guest_id, device_id
            FROM sessions
            WHERE token = ?
            """,
            (token,),
    ...
    if session["mode"] != expected_mode:
        delete_session(token)
        raise HTTPException(status_code=401, detail="Session mode is invalid. Please sign in again.")
    ...
    return {
        ...
        "guest_id": session["guest_id"] if expected_mode == "guest" else None,
...
def normalize_chat(chat_item: dict[str, Any]) -> dict[str, Any]:
    created_at = chat_item.get("created_at") or now_iso()
    updated_at = chat_item.get("updated_at") or created_at
    return {
...
def sort_chats(chats: list[dict[str, Any]]) -> list[dict[str, Any]]:
    newest_first = sorted(chats, key=lambda item: item.get("updated_at", ""), reverse=True)
...
def save_current_chat(
    ...
    without_current = [
        chat_item for chat_item in chats if chat_item["id"] != current_chat["id"]
    ]
    without_current.insert(0, normalize_chat(current_chat))
    return save_chats(profile_id, without_current)
...
def save_code_chats(profile_id: str, chats: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return load_code_chats(profile_id)
...
def save_current_code_chat(
    ...
    without_current.insert(0, normalize_chat(current_chat))
    return save_code_chats(profile_id, without_current)
...
def update_memory_from_message(profile_id: str, message: str) -> dict[str, Any]:
    memory = load_memory(profile_id)
    ...
    return save_memory(profile_id, memory)
...
def format_history(messages: list[dict[str, Any]], limit: int = 10) -> str:
    ...
    for item in messages[-limit:]:
        ...
        if text:
            ...
            history.append(f"{role}: {clip_text(str(text), 1200)}")
    return "\n".join(history)
...
def save_document_response(
    ...
    current_chat["messages"].append({"role": "ai", "text": error_message})
    save_current_chat(profile_id, chats, current_chat)
...
async def prepare_chat(
    ...
    assert_chat_owner_or_new(profile_id, chat_id)
    ...
    memory = update_memory_from_message(profile_id, user_message)
    uploaded_file = None
...
async def code_chat_stream(
    ...
    profile_id = profile["id"]
    assert_chat_owner_or_new(profile_id, chat_id, code=True)
    user_message = (message or "").strip() or "Help me write code."
    chats = load_code_chats(profile_id)
    current_chat = get_current_code_chat(chats, chat_id)
    if replace_last_turn:
        remove_latest_turn_for_retry(current_chat)
    ...
    if quick_reply:
        ...
        def quick_response_stream():
            ...
            threading.Thread(
                target=persist_code_response,
                args=(profile_id, chats, current_chat, user_message, quick_reply, task, language, saved_project_files, []),
                daemon=True,
            ).start()
    ...
    log_activity(profile_id, "code_request", {"task": task, "language": language})
    ...
    if clarification:
        def clarification_response_stream():
            ...
            threading.Thread(
                target=persist_code_response,
                args=(profile_id, chats, current_chat, user_message, clarification, task, language, relevant_project_files, []),
                daemon=True,
            ).start()
...
async def chat(
    ...
    log_activity(profile_id, "chat_request", {"intent": intent, "mode": answer_mode, "tool": route["tool"]})
...
async def chat_stream(
    ...
    if file_context.startswith(DOCUMENT_NOT_IN_CHAT_PREFIX):
        return JSONResponse(
            save_document_not_available_response(
                profile_id,
                chats,
                current_chat,
                user_message,
    ...
    answer_mode = route["answerMode"]
    log_activity(profile_id, "chat_stream_request", {"intent": intent, "mode": answer_mode, "tool": route["tool"]})
    has_history = bool(current_chat.get("messages"))
    clarification = clarification_response(
        user_message,
        intent,
    ...
    def response_stream():
        ...
        yield f"{META_PREFIX}{json.dumps(meta)}{META_SUFFIX}"
        threading.Thread(
            target=persist_ai_response,
            args=(profile_id, chats, current_chat, user_message, final_text),
            kwargs={
                "citations": citations,
                "document_hits": document_hits,
                "suggestions": suggestions,
